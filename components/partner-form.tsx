...
export default function PartnerForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    data.append("entry.FIELD_ID_3", formData.partnerType)
    data.append("entry.FIELD_ID_4", formData.city)
    data.append("entry.FIELD_ID_5", formData.mobileNumber)
    data.append("entry.FIELD_ID_6", formData.whatsappNumber)
    data.append("entry.FIELD_ID_7", formData.email)
    data.append("entry.FIELD_ID_8", formData.message)
  ...
  return (
    <div id="partner-form" className="w-full max-w-2xl mx-auto bg-white p-6 lg:p-8 rounded-xl shadow-lg border border-primary/10">
      ...
      <form onSubmit={handleSubmit} className="space-y-3 lg:space-y-4">
        ...
        <div>
          <label className="block text-xs lg:text-sm font-semibold text-foreground mb-1.5 lg:mb-2">Business / Clinic Name *</label>
          <input
            type="text"
            name="businessName"
        ...
        <div>
          ...
          >
            ...
            <option value="entrepreneur">Healthcare Entrepreneur</option>
          </select>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 lg:gap-4">
          <div>
            <label className="block text-xs lg:text-sm font-semibold text-foreground mb-1.5 lg:mb-2">City *</label>
            <input
              type="text"
              name="city"
              value={formData.city}
